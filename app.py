...
def start_ram_stress():
    try:
        ...
        subprocess.Popen(["stress", "--vm", "1", "--vm-bytes", "3G", "--timeout", "300"])
        return jsonify({'message': 'RAM stress started with 6GB for 300 seconds'}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def start_disk_write_stress():
    try:
        ...
        return jsonify({'message': 'Disk write stress started with 1GB for 300 seconds'}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
